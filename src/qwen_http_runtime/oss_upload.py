...
def subresource_string(params: dict[str, str]) -> str:
    entries = sorted((key, value) for key, value in params.items() if value is not None)
    if not entries:
        return ""
...
def sign_oss_request(
    *,
    ...
    string_to_sign = "\n".join([method, content_md5, content_type, date_value, canonical_headers + resource])
...
def initiate_multipart_upload(sts: dict[str, Any], mime_type: str) -> str:
    ...
    headers = {
        ...
        "x-oss-user-agent": "qwen-http-runtime/0.1",
    }
    ...
    req = urllib_request.Request(
        ...
        method="POST",
        headers={**headers, "authorization": authorization, "content-type": mime_type},
    )
...
def upload_part(sts: dict[str, Any], upload_id: str, part_number: int, chunk: bytes, mime_type: str) -> str:
    ...
    authorization = sign_oss_request(
        ...
        date_value=oss_date,
        oss_headers=headers,
        subresources={"partNumber": str(part_number), "uploadId": upload_id},
    )
    req = urllib_request.Request(
        build_oss_url(
            sts["bucket"],
        ...
        data=chunk,
        method="PUT",
        headers={**headers, "authorization": authorization, "content-md5": content_md5, "content-type": mime_type},
    )
    with _open_request(req) as response:
...
def complete_multipart_upload(sts: dict[str, Any], upload_id: str, parts: list[dict[str, Any]]) -> None:
    ...
    req = urllib_request.Request(
        build_oss_url(sts["bucket"], sts["endpoint"], sts["fileKey"], {"uploadId": upload_id}),
        data=xml,
        method="POST",
        headers={**headers, "authorization": authorization, "content-md5": content_md5, "content-type": content_type},
    )
    with _open_request(req):
        return
